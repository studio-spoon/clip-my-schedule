...
interface UseBufferSlotsParams {
...
export function useBufferSlots({
  selectedMembers,
  teamMembers,
  selectedPeriod,
  customPeriodStart,
  ...
  bufferTimeBefore,
  bufferTimeAfter
}: UseBufferSlotsParams) {
  const [bufferSlots, setBufferSlots] = useState<BufferSlot[]>([])
  ...
  useEffect(() => {
    ...
    const fetchBufferSlots = async () => {
      ...
      try {
        ...
        console.error('Buffer slots fetch error:', error)
