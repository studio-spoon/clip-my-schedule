  ...
  try {
    ...
    const busyTimesPromises = emails.map(async (email) => {
      try {
        ...
        const processedBusy = busyPeriods
          .map((period: { start?: string | null; end?: string | null }) => ({
            start: period.start || '',
            end: period.end || '',
          }))
          .filter(
        ...
        console.log(
          `✅ Fetched ${processedBusy.length} busy periods for ${email}`
        );
        if (processedBusy.length > 0) {
          console.log(
            `📅 Busy periods:`,
            processedBusy.map((p) => `${p.start} - ${p.end}`)
          );
        }
...
function calculateFreeSlots(
  ...
  for (
    ...
    console.log(
      `   Date details: ${date.toISOString()} (ISO), ${date.toString()} (toString)`
    );
    ...
    console.log(
      `   📋 Found ${dayBusyPeriods.length} busy periods for this date`
    );
    ...
    console.log(
      `   🕐 Work hours: ${workStart.toLocaleTimeString()} - ${workEnd.toLocaleTimeString()}`
    );
    ...
    if (daySlots.length > 0) {
      ...
      console.log(
        `  📋 Adding ${daySlots.length} meeting slots for ${dateStr}`
      );
